...
describe 'successfully run functional test' do
  it 'displays PASS and returns 0' do
    output = `sudo /tmp/security-agent/testsuite -test.v -status-metrics -loglevel trace --test.run 'TestOverlayFS|TestRulesetLoaded' 1>&2`
    retval = $?
...
if File.readlines("/etc/os-release").grep(/SUSE/).size == 0 and ! File.exists?('/etc/rhsm')
  describe 'successfully run functional test inside a container' do
    it 'displays PASS and returns 0' do
      output = `sudo docker exec -ti docker-testsuite /tmp/security-agent/testsuite -test.v -status-metrics -loglevel trace -env docker --test.run 'TestOverlayFS|TestRulesetLoaded' 1>&2`
      retval = $?
      expect(retval).to eq(0)
    end
  end
