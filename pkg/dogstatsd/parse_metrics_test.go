...
func TestParseCounterWithTags(t *testing.T) {
	...
	assert.Equal(t, "protocol:http", sample.tags[0].Text())
	assert.Equal(t, "bench", sample.tags[1].Text())
	assert.InEpsilon(t, 1.0, sample.sampleRate, epsilon)
}
...
func TestParseGaugeWithTags(t *testing.T) {
	...
	assert.Equal(t, "sometag1:somevalue1", sample.tags[0].Text())
	assert.Equal(t, "sometag2:somevalue2", sample.tags[1].Text())
	assert.InEpsilon(t, 1.0, sample.sampleRate, epsilon)
}
...
func TestParseGaugeWithUnicode(t *testing.T) {
	...
	assert.Equal(t, "intitulé:T0µ", sample.tags[0].Text())
	assert.InEpsilon(t, 1.0, sample.sampleRate, epsilon)
}
