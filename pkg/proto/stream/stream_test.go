...
func ExampleNew() {
	...
	const fieldFoo int = 1
	const fieldBar int = 2

	err := ps.Int32(fieldFoo, 42)
	if err != nil {
		panic(err)
	}
	err = ps.String(fieldBar, "forty-two")
	if err != nil {
		panic(err)
	}

	fmt.Printf("result: %#v", output.Bytes())
	// output:
	// result: []byte{0x8, 0x2a, 0x12, 0x9, 0x66, 0x6f, 0x72, 0x74, 0x79, 0x2d, 0x74, 0x77, 0x6f}
}
