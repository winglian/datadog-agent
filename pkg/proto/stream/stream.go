...
// String writes a string to the stream.  Note that the string must be copied
// to convert it to []byte, so where possible prefer the Bytes method.
...
// stream.
//
// NOTE: if the inner function creates an empty message, that empty message will still
...
// encodeVarint encodes the given 64-bit integer as a varint into the given
// buffer, returning it
func (ps *ProtoStream) encodeVarint(v uint64, scratch []byte) []byte {
	for v >= 1<<7 {
		scratch = append(scratch, uint8(v&0x7f|0x80))
		v >>= 7
	}
	scratch = append(scratch, uint8(v))
	return scratch
}
