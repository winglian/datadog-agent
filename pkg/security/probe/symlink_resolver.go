...
// rules compilations
func (s *SymlinkResolver) InitStringValues(key unsafe.Pointer, field eval.Field, values *eval.StringValues, state *eval.RuleState) {
	target := Targets{
		Field:  field,
		Values: values.Clone(),
	}
	...
	for _, value := range values.GetFieldValues() {
		state.UpdateFieldValues(field, value)
	}
	...
	for path, targets := range s.index {
		...
		for _, target := range targets {
			...
			if fieldValue != nil {
				...
				s.state.UpdateFieldValues(target.Field, *fieldValue)
			}
