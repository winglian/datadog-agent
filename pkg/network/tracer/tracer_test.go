...
func TestUDPSendAndReceive(t *testing.T) {
	...
	require.Equal(t, clientMessageSize, int(outgoing.SentBytes))
	require.Equal(t, serverMessageSize, int(outgoing.RecvBytes))
	require.True(t, outgoing.IntraHost)

	// make sure the inverse values are seen for the other message
	require.Equal(t, serverMessageSize, int(incoming.SentBytes))
	require.Equal(t, clientMessageSize, int(incoming.RecvBytes))
