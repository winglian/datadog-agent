...
func checkPolicies(cmd *cobra.Command, args []string) error {
	...
	opts := rules.NewOptsWithParams(model.SECLConstants, sprobe.SupportedDiscarders, enabled, sprobe.AllCustomRuleIDs(), model.SECLLegacyAttributes, nil, &securityLogger.PatternLogger{})
	model := &model.Model{}
