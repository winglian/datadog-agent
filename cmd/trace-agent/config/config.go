...
func applyDatadogConfig(c *config.AgentConfig) error {
	...
	if coreconfig.Datadog.IsSet("dogstatsd_pipe_name") {
		c.StatsdPipe = coreconfig.Datadog.GetString("dogstatsd_pipe_name")
	}
	if coreconfig.Datadog.IsSet("dogstatsd_socket") {
	...
	if host := coreconfig.Datadog.GetString("apm_config.apm_dd_url"); host != "" {
		c.Endpoints[0].Host = host
		if site != "" {
			log.Infof("'site' and 'apm_dd_url' are both set, using endpoint: %q", host)
		}
	}
	if coreconfig.Datadog.IsSet("apm_config.additional_endpoints") {
		for url, keys := range coreconfig.Datadog.GetStringMapStringSlice("apm_config.additional_endpoints") {
			if len(keys) == 0 {
				log.Errorf("'additional_endpoints' entries must have at least one API key present")
				continue
			}
			for _, key := range keys {
				key = coreconfig.SanitizeAPIKey(key)
				c.Endpoints = append(c.Endpoints, &config.Endpoint{Host: url, APIKey: key})
			}
		}
	}
	...
	if otlp.IsEnabled(coreconfig.Datadog) {
		grpcPort = coreconfig.Datadog.GetInt(coreconfig.ExperimentalOTLPTracePort)
	}
	...
	c.ProxyTransportFunc = func() func(*http.Request) (*url.URL, error) {
		if p := coreconfig.GetProxies(); p != nil {
			return httputils.GetProxyTransportFunc(p)
		}
...
func Load(path string) (*config.AgentConfig, error) {
