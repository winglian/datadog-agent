...
candle -arch x64 %wix_extension_switches% -dProjectSourceDir="c:\opt\datadog-dogstatsd" -dExtraEXAMPLECONFSLOCATION="C:\omnibus-ruby\src\etc\datadog-dogstatsd\extra_package_files\EXAMPLECONFSLOCATION" project-files.wxs source.wxs

if not "%ERRORLEVEL%" == "0" goto :done
light -ext WixUIExtension -ext WixBalExtension %wix_extension_switches% -cultures:en-us -loc localization-en-us.wxl project-files.wixobj source.wixobj  -out ddagent.msi
