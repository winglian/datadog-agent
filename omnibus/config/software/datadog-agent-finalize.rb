...
build do
    ...
    block do
        ...
        if windows?
            ...
            command "7z a -mx=5 -ms=on #{Omnibus::Config.source_dir()}\\etc\\datadog-agent\\extra_package_files\\embedded3.7z #{windows_safe_path(python_3_embedded)}"
            if with_python_runtime? "2"
                command "7z a -mx=5 -ms=on #{Omnibus::Config.source_dir()}\\etc\\datadog-agent\\extra_package_files\\embedded2.7z #{windows_safe_path(python_2_embedded)}"
            end
        end
