    ...
    block do
        ...
        if linux? || osx?
            ...
            if with_python_runtime? "2"
                ...
                link "embedded/bin/pip2", "#{install_dir}/embedded/bin/pip"

                delete "#{install_dir}/embedded/bin/2to3"
                link "embedded/bin/2to3-2.7", "#{install_dir}/embedded/bin/2to3"
            # Setup script aliases, e.g. `/opt/datadog-agent/embedded/bin/pip` will
            ...
            # Caution: we don't want to do this for Agent 6.x
                ...
                delete "#{install_dir}/embedded/bin/pip"
                link "embedded/bin/pip3", "#{install_dir}/embedded/bin/pip"

                delete "#{install_dir}/embedded/bin/python"
                link "embedded/bin/python3", "#{install_dir}/embedded/bin/python"

                delete "#{install_dir}/embedded/bin/2to3"
                link "embedded/bin/2to3-3.8", "#{install_dir}/embedded/bin/2to3"
            end
        end
